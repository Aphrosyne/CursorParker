...
- `光标停放器设置.cmd`：打开中文命令行菜单，显示运行、正在停止或未运行状态，并提供运行开关以及“启用开机启动”“禁用开机启动”两条明确指令。
...
开机启动通过当前用户“启动”目录里的 `CursorParker.lnk` 实现，不修改注册表，也不需要管理员权限。启用会创建或更新本项目的快捷方式，禁用只删除可识别为本项目的快捷方式。如果菜单无法通过目标、指向 `cursor_parker.ps1` 的 `-File` 参数和说明标记确认快捷方式归属，会提示手动检查并保留它；请核对快捷方式属性，或手动改名后再操作。程序和配置仍保留在本目录。
